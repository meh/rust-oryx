...
function M.setup()
    ...
    vim.api.nvim_create_autocmd("User", {
        ...
        callback = a.void(function(args)
            ...
            log("session " .. sid_short(sid) .. ': prompt "' .. title .. '" — awaiting keyboard')
            local accepted = entry.job:prompt(title)
            log("session " .. sid_short(sid) .. ": prompt → " .. (accepted and "accepted" or "rejected"))

            local api_ok, api = pcall(require, "opencode.api")
            if not api_ok then
                warn("session " .. sid_short(sid) .. ": prompt resolved but opencode.api unavailable")
                return
            end
            if accepted then
